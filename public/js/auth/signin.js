            ...
            $("#login_btn").click(async function (e) {
                ...
                try {
                    ...
                    if (sta == 200) {
                        ...
                        // call backend
                        const email = $('#signin_email').val(); // Get the email
                        let exchangeUrl = `${baseUrl}/exchange/${email}`; // Construct the URL

                        let backSave = await fetch(exchangeUrl, { method: 'GET' }); // Call /exchange
                        let b_status = backSave.status;

                        console.log(backSave);
                        if (b_status == 200) {
                            console.log('Email sent to /exchange');
                            // call backend
                            $('#call_error').text('');
                            $('#call_error').hide();
                            $('#call_success').show();
